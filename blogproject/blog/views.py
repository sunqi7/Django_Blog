...
class PostDetailView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        # 编写get方法的目的是因为每当文章被访问一次，就得将文章的阅读量+1
        # get方法返回的是一个HttpResponse实例
        # 之所以需要调用父类的get方法，是因为只有当get方法被调用后，
        # 才会有self.object属性，其值为Post模型实例，即被访问的文章post
    ...
    def get_context_data(self, **kwargs):
        # 编写get_context_data的目的是因为除了将post传递给模板外<DetailView已经帮我们完成了>
        # 还有将表单评论，post下的评论列表传递给模板
...
class archivesView(ListView):
    ...
    def get_queryset(self):
        ...
        return super(archivesView, self).get_queryset().filter(creatd_time__year=year, creatd_time__month=month)
